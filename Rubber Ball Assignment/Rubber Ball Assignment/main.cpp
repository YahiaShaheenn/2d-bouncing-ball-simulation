...
void drawScene(void)
{
	...
	for (int z = -15; z <= 15; z += 1)
	{
		...
		glVertex3f(45.0, 0.0, -z);
	}

	for (int x = -15; x <= 45; x += 1)
	{
	...
	glTranslatef(h*t, v*t_bounce - (g / 2.0)* t_bounce * t_bounce, 0.0);
...
void animate(int value)
{
	if (isAnimate)
	{
		...
		if (v * t_bounce - (g / 2.0) * t_bounce * t_bounce <= 0 && t_bounce >= 1.0)
		{
			v = v * -0.7;
			t_bounce = 0.0;
		}
